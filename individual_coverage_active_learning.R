...
all_gameIds = unique(all_offense_lookup$gameId)
...
# my_playId = all_playIds[59]


for(ii in 1:length(all_gameIds)){
  stoppage = FALSE
  my_gameId = all_gameIds[ii]
  all_playIds = unique(all_offense_lookup$playId[which(all_offense_lookup$gameId == my_gameId)])
  
  for(jj in 1:length(all_playIds)){
    my_playId = all_playIds[jj]
    
    # load in individualCoverage tibble
    individual_coverage_filename = 'C:/Users/Mitch/PycharmProjects/BDB_revisit/Data/individualCoverage.csv'
    individual_coverage_tibble = read_csv(individual_coverage_filename, col_types = cols())
    
    if(my_gameId %in% individual_coverage_tibble$gameId & my_playId %in% individual_coverage_tibble$playId){
      next
    }
    
    # get relevant files with current gameId rows, need to reset after each play
    ...
    for(ni in non_blitz_nflIds){
      ...
      if(nrow(individual_coverage_tibble) <= threshold){
        ...
        svm_prob = attr(predict(models$SVM, out_individual_coverages[ni_row, c('averageClosestReceiver', 'totalDistance', 'losDifference', 'uniqueClosestReciever')], probability=TRUE), 'probabilities')[,1]
    ...
    if(length(confidence_probabilities) > 1){
      ...
      if(min(confidence_probabilities) < 0.10){
    ...
    cat('The correct percentage in training is: ', round(correct_, 4), '\n')
